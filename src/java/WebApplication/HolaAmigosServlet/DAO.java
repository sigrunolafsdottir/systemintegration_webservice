...
public class DAO {
   ...
   public String getPersonByNameAsString(String s){
       for (Person p :allPersons){
           if (p.getName().equalsIgnoreCase(s)){
               System.out.println("dao: "+p.name+" "+p.address+" "+p.getBirthday().toString());
               return p.name+" "+p.address+" "+p.getBirthday().toString();
           }
       }
       System.out.println("returning null in dao");
       return null;
   }
